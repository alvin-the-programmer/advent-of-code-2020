...
const getNeighbors = (node) => {
  const [ x, y, z ] = tripletFromNode(node);
  const neighbors = new Set();
  const deltas = [-1, 0, 1];
  for (let xDelta of deltas) {
    for (let yDelta of deltas) {
      for (let zDelta of deltas) {
        const neighbor = nodeFromTriplet([x + xDelta, y + yDelta, z + zDelta]);
...
const parseInitialNodes = (lines) => {
  ...
  for (let x = 0; x < lines.length; x += 1) {
    for (let y = 0; y < lines[0].length; y += 1) {
      let nodeKey = nodeFromTriplet([x, y, 0]);
      if (lines[x][y] === '#')
      set.add(nodeKey);
    }
  }
